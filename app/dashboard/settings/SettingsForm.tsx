...
export function SettingsForm({ profile, userEmail, userId }: Props) {
  ...
  const usernameHint = () => {
    ...
    if (usernameStatus === "taken") return <span style={{ color: "#E5484D" }}>✗ Taken</span>;
    if (usernameStatus === "invalid") return <span style={{ color: "#E5484D" }}>3–20 chars, letters/numbers/underscore only</span>;
    return null;
  };
    ...
    <div className="flex flex-col gap-10">
      ...
      <section>
        ...
        <div className="mb-8">
            ...
            >
              {!bannerUrl && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <p className="font-mono text-xs" style={{ color: "var(--ink-faint)" }}>No banner set</p>
                </div>
              )}
            ...
            {bannerUploadError && (
              <p className="font-sans text-[12px]" style={{ color: "#E5484D" }}>{bannerUploadError}</p>
            )}
        ...
        <div className="flex flex-col gap-5">
          ...
          <FieldGroup
            label="Bio"
            hint={
              <span style={{ color: bio.length > 150 ? "#E5484D" : "var(--ink-faint)" }}>
                {bio.length}/160
              </span>
            }
          >
...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h2
      ...
      style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", color: "var(--ink-faint)" }}
    >
      {children}
    </h2>
  );
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          style={{
            background: copied ? "var(--ink)" : "var(--line)",
            border: "1px solid transparent",
            color: copied ? "var(--bg)" : "var(--ink-muted)",
          }}
        >
          {copied ? "Copied ✓" : "Copy"}
        </button>
