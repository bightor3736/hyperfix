const pillars = [
  { title: "Log it.", body: "Name the obsession. Set today's intensity." },
  { title: "Count it.", body: "The counter starts the moment you commit." },
  { title: "Share it.", body: "Export a clean card. Drop it in a Story." },
  { title: "Mourn it.", body: "When it ends, write the eulogy." },
];
    ...
    <section id="features" style={{ background: "var(--bg)" }}>
      <div className="mx-auto max-w-[1200px] px-6 py-24 sm:px-10 sm:py-28">
        ...
        <div className="mt-14 grid grid-cols-1 gap-y-10 sm:grid-cols-2 sm:gap-x-12 lg:grid-cols-4 lg:gap-x-10">
          {pillars.map((p) => (
            <article key={p.title}>
              <h3 className="font-display text-[28px] leading-[1.1] tracking-tight" style={{ color: "var(--ink)" }}>
                {p.title}
              </h3>
              <p className="mt-3 text-[14px] leading-[1.6]" style={{ color: "var(--ink-muted)" }}>
                {p.body}
              </p>
            </article>
          ))}
