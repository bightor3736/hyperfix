...
async function middleware(request) {
    ...
    if (user && (pathname.startsWith("/dashboard") || pathname === "/onboarding") && !pathname.startsWith("/onboarding/username") && !pathname.startsWith("/api/")) {
        const { data: profile } = await supabase.from("profiles").select("username").eq("id", user.id).single();
        if (profile && !profile.username) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$esm$2f$server$2f$web$2f$spec$2d$extension$2f$response$2e$js__$5b$middleware$2d$edge$5d$__$28$ecmascript$29$__["NextResponse"].redirect(new URL("/onboarding/username", request.url));
        }
    }
